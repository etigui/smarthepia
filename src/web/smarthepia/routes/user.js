...
router.get('/', function(req, res, next) {
    if(auth.checkAuth(req, auth.getAdmin())){
        return res.render('pages/user', { lastname: req.session.lastname, dateTime: dateFormat(new Date(), "hh:MM:ss dd-mm-yyyy"),permission: req.session.permissions, page: "users", type:  req.query.type, message:  req.query.message});
    }else{
        return res.redirect('/');
    }
});
