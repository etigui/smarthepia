...
global IDs
#IDs=["5/98","5/99","5/100"]
#IDs=["4/1","4/2","4/3","4/10","4/11","4/12", "5/99", "5/98", "5/100"]
IDs=["5/98","5/99","5/100"]
...
def update_ids():
	...
	db_status, db = db_connect()
	new_ids = []
	if(db_status):
		actuators = db.sh.devices.find({"type": "Actuator"})
		for actuator in actuators:
			new_ids.append(str(actuator['address']))
		IDs.clear()
		IDs = new_ids.copy()
		return jsonify(status=0, result="Ids updated")
	else:
		return jsonify(status=1, result="Ids not updated (db connection error)")


@app.route("/print", methods=['GET'],strict_slashes=False)
def print_ids():
	return jsonify(status=0, result=IDs)
...
def db_connect():
	try:
		return True, pymongo.MongoClient(db_ip, db_port)
