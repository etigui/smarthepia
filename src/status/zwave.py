...
def check_zwave_network():
    ...
    for db_rpi in db_sensors:
        ...
        if utils.send_ping(db_rpi.ip):
            status.setdefault(db_rpi.rpi, []).append({"status": 0, "path":db_rpi.rpi, "reason": "RPI available"})

            # Check REST server (ping)
            if utils.rest_server_status(db_rpi.ip + ":" + db_rpi.port):
                status.setdefault(db_rpi.rpi, []).append({"status": 0, "path":db_rpi.rpi, "reason": "REST server available"})
                ...
                if result:
                    status.setdefault(db_rpi.rpi, []).append({"status": 0, "path":db_rpi.rpi, "reason": "Route get_nodes_list available"})
                            ...
                            if db_sensor['sensor_id'] in rest_sensors:
                                ...
                                if result:
                                    ...
                                    if rest_info.find('battery') != -1:
                                        ...
                                        if (rest_info['controller'].replace("Pi ", "rpi") == db_rpi.rpi) and (rest_info['location'] == db_sensor['location']):
                                            ...
                                            status.setdefault(db_rpi.rpi, []).append({"status": 1, "path": error_device, "reason": "Sensor id from REST server not match with the RPI DB"})
                                    elif rest_info.find('Node not ready or wrong sensor node type') != -1:
                                        status.setdefault(db_rpi.rpi, []).append({"status": 1, "path": error_device, "reason": "Sonsor not available"})
                                    else:
                                        status.setdefault(db_rpi.rpi, []).append({"status": 1, "path": error_device, "reason": "Sonsor error or not available"})
                                else:
                                    status.setdefault(db_rpi.rpi, []).append({"status": 1, "path": error_device, "reason": "Route get_all_measures not available"})
                            else:
                                status.setdefault(db_rpi.rpi, []).append({"status": 1, "path": error_device, "reason": "Sensor id not found (REST server)"})
                else:
                    status.setdefault(db_rpi.rpi, []).append({"status": 1, "path": db_rpi.rp, "reason": "Route get_nodes_list not available"})
            else:
                status.setdefault(db_rpi.rpi, []).append({"status": 1, "path": db_rpi.rp, "reason": "REST server not available"})
        else:
            status.setdefault(db_rpi.rpi, []).append({"status": 1, "path": db_rpi.rp, "reason": "RPI not available"})
    ...
    for key, values in status.items():
        ...
        print(f"RPI: {key}")
        for value in values:
                print(f"[{value['status']}] [{value['path']}] [{value['reason']}]")
        print('')
...
def get_db_sensors(mdb_host):
    ...
    sensors = client.smarthepia.multisensors.find({}, {'_id': False, 'rpi': True, 'ip': True, 'port': True, 'location': True, 'sensor_id': True, 'status': True, 'type': True})

    # Add sensor in dico with key (rpi,ip,port) and list of sensor id
    for sensor in sensors:
        key = f"{sensor['rpi']};{sensor['ip']};{sensor['port']}"
        #values = [sensor['location'], sensor['sensor_id'], sensor['status'], sensor['type']]
        values = {"location": sensor['location'], "sensor_id": sensor['sensor_id'], "status": sensor['status']}
